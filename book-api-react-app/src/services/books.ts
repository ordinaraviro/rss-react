...
export const booksApi = createApi({
    ...
    endpoints: (builder) => ({
        getBooksBySearchText: builder.query<BooksResponse, SearchBooksParams>({
            query: ({searchText, page}) => `search.json?q=${searchText}&page=${page}&limit=10&fields=title,author_name,cover_edition_key,edition_key,first_publish_year,first_sentence`,
        }),
    }),
})
